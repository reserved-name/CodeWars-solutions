function formatDuration (seconds) {
  ...
  let secunda = Math.floor(seconds % 60);
	secunda = (secunda === 0) ? '' : (secunda > 1) ? secunda + ' seconds' : secunda + ' second';
  return (seconds === 0) ? 'now' :
  [years + ', ' + days + ', ' + hours + ', ' + minutes + ', ' + secunda].toString().match(/[0-9]+\s(\w+)/gm).join(', ').replace(/,[^,](?!.*,)/, ' and ');
}
